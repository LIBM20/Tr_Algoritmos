...
from GestaoReunioes.GerirAssuntos.GerirTarefas.PesquisaPessoas import *
...
def Eliminar(dados):
    ...
    if pos == -1:
        ...
        if pos1 == -1:
            ...
            tarefa = input("ID da Tarefa? ")
            pos2 = PesquisaReuniaoAssuntoTarefasPorID(dados,pos,pos1, tarefa)
            if pos2 == -1:
                print("\033[0.91mNão encontrado\033[0m")
            else:
                pessoa = LerNumero2("Id do Subscritor ? ",1700000,1709999)
                pos3 = PesquisaPessoas(dados,pos,pos1,pos2,pessoa)
                if pos3 == -1:
                    print("\033[0.91mNão encontrado\033[0m")
                else:
                    print(dados['Reunioes'][pos]['Assuntos'][pos1]['Tarefas'][pos2]['Pessoas'][pos3])
                    op = LerNumero("1-Eliminar? 2-Cancelar a Eliminação",1,2,"inteiro")
                    if op == 1:
                        del(dados['Reunioes'][pos]['Assuntos'][pos1]['Tarefas'][pos2]['Pessoas'][pos3])
